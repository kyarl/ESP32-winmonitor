...
void loop() {
    ...
    const size_t totalBytes = (WIDTH * HEIGHT + 7) / 8;
    ...
    while (client.connected()) {
        ...
        if (available > 0) {
            ...
            size_t chunkSize = min((size_t)512, remaining);
            ...
            for (int bit = 7; bit >= 0; bit--) {
                pixelBuffer[pixelCount++] =
                    (b & (1 << bit)) ? 0xFFFF : 0x0000;
            }
            }

            size_t remainingPixels = WIDTH * HEIGHT - (receivedBytes * 8);
